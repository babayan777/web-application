        ...
        try {
            ...
            if (data.message) {
                ...
                if (data.failedFields.length === 1) {
                    setMessageState(`${data.failedFields[0]} is already exists. Please change it.`);
                } else if (data.failedFields.length === 2) {
                    setMessageState(`${data.failedFields[0]} and ${data.failedFields[1]} are already exists. Please change it.`);
                }
            }
        } catch (e) {
            console.log(e);
    ...
    return (
        <div style={{width:"100%",height:'100vh'}}>
            <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent:'center', width:'400px', height:'100vh',margin:'0 auto'}}>
                ...
                <Box component="span" sx={{ color: 'red', marginTop:'10px' }}>{userError  ? 'Username must contain uppercase, lowercase, number, from 6 to 20 characters ' : ''}</Box>
                <TextField label="First Name" variant="standard" style={{marginTop:'30px'}} onChange={firstNameChange}/>
                <Box component="span" sx={{ color: 'red', marginTop:'10px' }}>{firstNameError  ? 'First name cannot be empty' : ''}</Box>
                <TextField label="Last Name" variant="standard" style={{marginTop:'30px'}} onChange={lastNameChange}/>
                <Box component="span" sx={{ color: 'red', marginTop:'10px' }}>{lastNameError  ? 'Last name cannot be empty' : ''}</Box>
                <TextField label="Password" type="password" variant="standard" style={{marginTop:'30px'}} value={password} onChange={passChange}  />
                <Box component="span" sx={{ color: 'red', marginTop:'10px' }}>{passError  ? 'Password must contain uppercase, lowercase, number, from 6 to 20 characters' : ''}</Box>
                ...
                <Box component="span" sx={{ color: 'orange', marginTop:'10px' }}>{messageState}</Box>
